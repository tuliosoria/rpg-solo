...
const SINGULAR_EVENTS: SingularEvent[] = [
  ...
  },
  {
    ...
    trigger: (state, _command, _args) => {
      ...
      // Must have at least 1 evidence file found and warning shown (but we make warning trigger first)
      if (!state.tutorialComplete) return false;
      return (
  ...
  {
    // THE WATCHER ACKNOWLEDGMENT - After discovering 3+ evidence files, system acknowledges awareness
...
// Check if an action is "meaningful" (reading files, finding evidence)
function isMeaningfulAction(
  ...
  // Discovering evidence is definitely meaningful
...
function getContextualExplorationHints(state: GameState): string | null {
  ...
  if (!hasReadStorage && evidenceCount < 1) {
    return 'check /storage/ for transport logs.';
  }

  if (!hasReadOps && evidenceCount < 2) {
    return '/ops/ has quarantine records.';
  }

  if (!hasReadComms && evidenceCount < 3) {
  ...
  if (!hasReadAdmin && evidenceCount >= 2 && state.accessLevel >= 3) {
...
function getUFO74FileReaction(
  ...
  const isGettingParanoid = evidenceCount >= 3 || messageCount >= 6;
  const isAboutToFlee = evidenceCount >= 4 || messageCount >= 9;
  ...
  if (
    ...
    contextChance(state, 0.5, 'ufo74-degraded-trust', trustLevel, filePath, messageCount, evidenceCount)
  ) {
    messages = getUFO74DegradedTrustMessage(state, trustLevel, filePath);
    return messages;
  }

  // Check for conditional dialogue based on evidence discovery progress
  const conditionalDialogue = getUFO74ConditionalDialogue(state, filePath);
  if (
    conditionalDialogue &&
    contextChance(state, 0.6, 'ufo74-conditional-dialogue', filePath, messageCount, evidenceCount)
  ) {
...
function getUFO74ConditionalDialogue(state: GameState, filePath: string): TerminalEntry[] | null {
  ...
  // If 4+ evidence files are found, UFO74 gets excited
  if (evidenceCount >= 4) {
...
function getUFO74NoticeExplanation(notices: TerminalEntry[]): TerminalEntry[] | null {
  ...
  for (const notice of noticeTexts) {
    ...
    if (notice.includes('Evidence archive updated') || notice.includes('archive updated')) {
      explanations.push(createEntry('ufo74', 'UFO74: good. keep stacking proof.'));
      break;
    }
    if (notice.includes('Leak package almost ready') || notice.includes('almost ready')) {
      explanations.push(createEntry('ufo74', 'UFO74: almost there.'));
      break;
    }
    if (notice.includes('LEAK PACKAGE READY') || notice.includes('Leak package ready')) {
      explanations.push(createEntry('ufo74', 'UFO74: thats it. send it.'));
      break;
    }
  }
...
function getIncognitoMessage(
  ...
  // Only react to files that reveal evidence or when player is struggling.
  // Check if this is an evidence discovery moment
  const isEvidenceDiscovery =
    notices &&
    notices.some(
      n =>
  ...
  // unless it's an evidence discovery moment (those deserve celebration)
  if (!isEvidenceDiscovery && !isFirstUnstable) {
...
const commands: Record<string, (args: string[], state: GameState) => CommandResult> = {
  ...
  status: (args, state) => {
    ...
    if (evidenceCount >= MAX_EVIDENCE_COUNT) {
      ...
      lines.push('  OBJECTIVE: Keep reading. Each new evidence file counts.');
    } else {
  ...
  open: (args, state) => {
    ...
    // Check for evidence and disturbing content
    ...
    // Track broad content categories for ambient hinting
    ...
    // After a few reads, nudge players toward the main evidence directories.
    ...
    if (evidenceCount === 0) {
      return {
        output: [
          ...
          createEntry('system', ''),
          createEntry('system', '  Read evidence files to log them.'),
          createEntry('system', ''),
          createEntry('system', '╚═══════════════════════════════════════════════════════╝'),
        ],
        stateChanges: {},
...
export function executeCommand(input: string, state: GameState): CommandResult {
  ...
  if (state.godMode && lowerInput.startsWith('god ')) {
    ...
    if (godCmd === 'status') {
      const evidCount = countEvidence(state);
      return {
        output: [
