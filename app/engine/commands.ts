...
// ═══════════════════════════════════════════════════════════════════════════
// UFO74 DECRYPT HINTS SYSTEM - Help players with security question answers
// ═══════════════════════════════════════════════════════════════════════════

interface DecryptHintEntry {
  // Keywords that trigger this hint (in the question)
  keywords: string[];
  // The encrypted file this hint is for
  encryptedFile: string;
  // The source file that contains the answer
  sourceFile: string;
  // The actual answer
  answer: string;
  // Response when player HAS read the source file
  answerResponse: string[];
  // Response when player has NOT read the source file
  hintResponse: string[];
}

const DECRYPT_HINTS: DecryptHintEntry[] = [
  {
    keywords: ['weight', '340', 'material', 'sample', 'psi_comm', 'transport', 'kg'],
    encryptedFile: '/comms/psi/psi_comm_transcript.enc',
    sourceFile: '/storage/assets/transport_log_96.txt',
    answer: '340',
    answerResponse: [
      'UFO74: transport log says 340kg. thats your answer.',
      '       material sample weight.',
    ],
    hintResponse: [
      'UFO74: havent found that yet?',
      '       check transport_log_96.txt in /storage/assets/',
      '       weight is in there.',
    ],
  },
  {
    keywords: ['subjects', 'recovered', 'total', 'how many', 'three', '3', 'transcript_limit', 'bio_container'],
    encryptedFile: '/comms/psi/transcript_limit.enc',
    sourceFile: '/storage/quarantine/bio_container.log',
    answer: '3',
    answerResponse: [
      'UFO74: three subjects. alfa, beta, gamma.',
      '       bio_container.log has the designations.',
    ],
    hintResponse: [
      'UFO74: subject count is in quarantine records.',
      '       check bio_container.log in /storage/quarantine/',
    ],
  },
  {
    keywords: ['protocol', '7-echo', 'echo', 'exchange', 'governs', 'liaison', 'foreign'],
    encryptedFile: '/comms/liaison/diplomatic_exchange_log.enc',
    sourceFile: '/comms/liaison/foreign_liaison_note.txt',
    answer: '7-ECHO',
    answerResponse: [
      'UFO74: protocol 7-ECHO. found it in the liaison note.',
      '       governs the whole exchange.',
    ],
    hintResponse: [
      'UFO74: protocol name is in the diplomatic files.',
      '       check foreign_liaison_note.txt in /comms/liaison/',
    ],
  },
  {
    keywords: ['alfa', 'alpha', 'designation', 'subject', 'specimen', 'autopsy', 'neural_dump'],
    encryptedFile: '/storage/quarantine/neural_dump_raw.psi',
    sourceFile: '/storage/quarantine/autopsy_alpha.log',
    answer: 'ALFA',
    answerResponse: [
      'UFO74: subject ALFA. first one they got.',
      '       autopsy records have the full designation.',
    ],
    hintResponse: [
      'UFO74: subject designation is in autopsy records.',
      '       check autopsy_alpha.log in /storage/quarantine/',
    ],
  },
  {
    keywords: ['2026', 'year', 'transition', 'window', 'projected', 'signal_intercept'],
    encryptedFile: '/admin/signal_intercept.enc',
    sourceFile: 'multiple',
    answer: '2026',
    answerResponse: [
      'UFO74: 2026. the transition window.',
      '       its everywhere if you look.',
    ],
    hintResponse: [
      'UFO74: 2026 comes up a lot in the classified docs.',
      '       keep reading. youll find it.',
    ],
  },
];

/**
 * Check if a question is asking for decrypt/password help and return UFO74's response
 * based on whether the player has read the source file.
 */
function getUFO74DecryptHint(
  question: string,
  filesRead: Set<string>
): { isDecryptQuestion: boolean; response: TerminalEntry[] } {
  const q = question.toLowerCase();
  
  // Primary decrypt-related keywords that strongly indicate a password/decrypt question
  const strongDecryptKeywords = [
    'password', 'decrypt', 'encrypted', 'security question', 
    'unlock', 'stuck on', 'cant decrypt', "can't decrypt",
    'hint for', 'clue for', 'answer for',
  ];
  
  // Check if this question contains strong decrypt indicators
  const hasStrongDecryptIndicator = strongDecryptKeywords.some(kw => q.includes(kw));
  
  // Patterns that indicate the player is asking about something specific
  const askingPatterns = [
    'what is', 'whats', "what's", 'what protocol', 'what was',
    'how many', 'how much', 
    'tell me the', 'give me the',
    'need the', 'need to know',
  ];
  const isAskingQuestion = askingPatterns.some(p => q.includes(p));
  
  // Find which decrypt hint matches the question based on specific file/answer keywords
  for (const hint of DECRYPT_HINTS) {
    const matchesHint = hint.keywords.some(kw => q.includes(kw.toLowerCase()));
    if (matchesHint) {
      // This matches a specific hint - check if it's actually asking about decryption
      // Allow specific keywords to trigger even without strong decrypt indicator
      // (e.g., "what is the weight" is valid)
      const specificFileKeywords = ['psi_comm', 'transcript_limit', 'neural_dump', 'signal_intercept', 'diplomatic', '.enc', '.psi'];
      const mentionsSpecificFile = specificFileKeywords.some(kw => q.includes(kw));
      
      // Need either a strong decrypt indicator OR a specific file mention OR asking a question with hint keyword
      if (!hasStrongDecryptIndicator && !mentionsSpecificFile && !isAskingQuestion) {
        // This might be a false positive - the keyword appears but not in decrypt context
        continue;
      }
      
      // Check if player has read the source file
      const hasReadSource = hint.sourceFile === 'multiple' 
        ? true // For answers found in multiple places, always give answer
        : Array.from(filesRead).some(f => f.includes(hint.sourceFile.split('/').pop() || ''));
      
      const response = hasReadSource ? hint.answerResponse : hint.hintResponse;
      
      return {
        isDecryptQuestion: true,
        response: response.map(line => createEntry('ufo74', line)),
      };
    }
  }
  
  // Only return generic help if there's a strong decrypt indicator
  if (hasStrongDecryptIndicator) {
    return {
      isDecryptQuestion: true,
      response: [
        createEntry('ufo74', 'UFO74: passwords are hidden in other files.'),
        createEntry('ufo74', '       read everything. answers are there.'),
        createEntry('ufo74', '       check the hints in the security prompts.'),
      ],
    };
  }
  
  // Not a decrypt question
  return { isDecryptQuestion: false, response: [] };
}
...
const commands: Record<string, (args: string[], state: GameState) => CommandResult> = {
  ...
  chat: (args, state) => {
    ...
    // Check if this is a decrypt/password question - route to UFO74 instead of Prisoner 45
    const decryptHint = getUFO74DecryptHint(question, state.filesRead || new Set());
    if (decryptHint.isDecryptQuestion) {
      return {
        output: [
          createEntry('input', `> ${question}`),
          createEntry('system', ''),
          createEntry('system', '─────────────────────────────────────────'),
          createEntry('system', 'UFO74 INTERCEPT'),
          createEntry('system', '─────────────────────────────────────────'),
          createEntry('system', ''),
          ...decryptHint.response,
          createEntry('system', ''),
        ],
        // No state changes - doesn't count as a Prisoner 45 question
        stateChanges: {},
        delayMs: 800,
      };
    }
