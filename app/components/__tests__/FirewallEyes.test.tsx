...
import FirewallEyes, { DETECTION_THRESHOLD } from '../FirewallEyes';
...
describe('FirewallEyes (ambient)', () => {
  ...
  it('renders 2 eyes at detection 30%', () => {
    const { container } = render(
      <FirewallEyes {...baseProps} firewallActive={true} detectionLevel={30} />
    );
    expect(container.querySelectorAll('[data-testid="firewall-ambient-eye"]')).toHaveLength(2);
  });

  it('renders 4 eyes at detection 50%', () => {
    const { container } = render(
      <FirewallEyes {...baseProps} firewallActive={true} detectionLevel={50} />
    );
    expect(container.querySelectorAll('[data-testid="firewall-ambient-eye"]')).toHaveLength(4);
  });

  it('renders 6 eyes at detection 70%', () => {
    const { container } = render(
      <FirewallEyes {...baseProps} firewallActive={true} detectionLevel={70} />
    );
    expect(container.querySelectorAll('[data-testid="firewall-ambient-eye"]')).toHaveLength(6);
  });

  it('renders 8 eyes at detection 85%', () => {
    const { container } = render(
      <FirewallEyes {...baseProps} firewallActive={true} detectionLevel={85} />
    );
    expect(container.querySelectorAll('[data-testid="firewall-ambient-eye"]')).toHaveLength(8);
  });

  it('renders 10 eyes at detection 95%', () => {
    const { container } = render(
      <FirewallEyes {...baseProps} firewallActive={true} detectionLevel={95} />
    );
    expect(container.querySelectorAll('[data-testid="firewall-ambient-eye"]')).toHaveLength(10);
  });
  ...
  it('container has the firewallContainer CSS class', () => {
    ...
    expect(container.querySelector('.' + styles.firewallContainer)).not.toBeNull();
  });
